...
function keysAndValues(obj) {
  ...
  const result = [objKeys, objValues];
  return result;
}
...
function filterObjectsByKey(arr, key) {
  ...
  const filterArr = arr.filter((value) => value.hasOwnProperty(key));
...
function getObjectValuesSum(obj) {
  ...
  const sum = objValues.reduce((elem1, elem2) => elem1 + elem2);
  // Повертаємо суму
  return sum;
}
